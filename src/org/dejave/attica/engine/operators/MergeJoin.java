 ...
 * 	and I have a feeling wouldn't work well with some types of the operators. If it was possible, one could use {@link SortMerger} class.
 *  (if we had access to the intermediate files directly (after the sort phase), we could modify tuples() and pages() iterators (as in RelationalIOManager._java and Page._java (the test in Main() in RelationalIOManager2.java works))
 * 	and then work one these files directly. We have this access (by calling first tuple from operators and then tuple.getTupleIdentifier().getFileName() - however I didn't do that, since it breaks cohesion and is a hack).
